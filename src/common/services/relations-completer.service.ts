  ...
  private async completeRelations(dto: CreateDto | UpdateDto, entity: T): Promise<T> {
    if ('people' in entity && 'people' in dto && dto.people != null) {
      entity.people = await Promise.all(
        dto.people.map((id) => this.personRepository.findOneByOrFail({ id })),
      );
    }

    if ('pilots' in entity && 'pilots' in dto && dto.pilots != null) {
      entity.pilots = await Promise.all(
        dto.pilots.map((id) => this.personRepository.findOneByOrFail({ id })),
      );
    }

    if ('characters' in entity && 'characters' in dto && dto.characters != null) {
      entity.characters = await Promise.all(
        dto.characters.map((id) => this.personRepository.findOneByOrFail({ id })),
      );
    }
