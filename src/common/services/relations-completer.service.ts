...
export class RelationsCompleterService<T extends ExistingEntity> {
    ...
    private async completeRelations(dto: CreateDto | UpdateDto, entity: T): Promise<T> {
        if ("people" in entity && "people" in dto && dto.people != null) {
            const personRepository: Repository<Person> = this.dataSource.getRepository<Person>(Person)
            entity.people = await Promise.all(dto.people.map(id => personRepository.findOneByOrFail({id})));
        }

        if ("pilots" in entity && "pilots" in dto && dto.pilots != null) {
            const personRepository: Repository<Person> = this.dataSource.getRepository<Person>(Person)
            entity.pilots = await Promise.all(dto.pilots.map(id => personRepository.findOneByOrFail({id})));
        }

        if ("characters" in entity && "characters" in dto && dto.characters != null) {
            const personRepository: Repository<Person> = this.dataSource.getRepository<Person>(Person)
            entity.characters = await Promise.all(dto.characters.map(id => personRepository.findOneByOrFail({id})));
        }

        if ("homeworld" in entity && "homeworld" in dto && dto.homeworld != null && Array.isArray(dto.homeworld)) {
            const planetRepository: Repository<Planet> = this.dataSource.getRepository<Planet>(Planet)
            entity.homeworld = await planetRepository.findOneByOrFail({id: dto.homeworld})
        }

        if ("films" in entity && "films" in dto && Array.isArray(dto.films)) {
            const filmRepository: Repository<Film> = this.dataSource.getRepository<Film>(Film)
            entity.films = await Promise.all(dto.films.map(id => filmRepository.findOneByOrFail({id})));
        }

        if ("species" in entity && "species" in dto && Array.isArray(dto.species)) {
            const specieRepository: Repository<Specie> = this.dataSource.getRepository<Specie>(Specie)
            entity.species = await Promise.all(dto.species.map(id => specieRepository.findOneByOrFail({id})))
        }

        if ("starships" in entity && "starships" in dto && Array.isArray(dto.starships)) {
            const starshipRepository: Repository<Starship> = this.dataSource.getRepository<Starship>(Starship)
            entity.starships = await Promise.all(dto.starships.map(id => starshipRepository.findOneByOrFail({id})));
        }

        if ("vehicles" in entity && "vehicles" in dto && Array.isArray(dto.vehicles)) {
            const vehicleRepository: Repository<Vehicle> = this.dataSource.getRepository<Vehicle>(Vehicle)
