...
export class RelationsToUrisInterceptor implements NestInterceptor {
  ...
  intercept(
    ...
    return next.handle().pipe(
      map(
        (findManyResult: ExistingEntity | ExistingEntity[]): ExistingEntity | ExistingEntity[] => {
  ...
  private mapArrayishRelations<T extends ExistingEntity>(relations: T[]): string[] {
    return relations.map((relation: ExistingEntity) => {
      const tableName: string = this.dataSource.getMetadata(relation.constructor).tableName;
  ...
  private isNonEmptyHomeworldRelation<T extends ExistingEntity>(item: T, prop: string): boolean {
    return (
      prop == STR_PROPS_TO_IGNORE.homeworld && item[prop] != null && typeof item[prop] === 'object'
    );
  }

  private formURI(tableName: string, id: number): string {
    const { protocol, host, port } = this.configService.getOrThrow<AppEnvVars>('app');
