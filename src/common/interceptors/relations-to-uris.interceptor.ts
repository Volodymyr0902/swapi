...
export class RelationsToUrisInterceptor implements NestInterceptor {
    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<ExistingEntity | ExistingEntity[]> {
        return next.handle().pipe(
            map((findManyResult: ExistingEntity | ExistingEntity[]): ExistingEntity | ExistingEntity[] => {
    ...
    private isNonEmptyHomeworldRelation<T extends ExistingEntity>(item: T, prop: string): boolean {
        return prop === STR_PROPS_TO_IGNORE.homeworld && item[prop] != null && typeof item[prop] === 'object';
    }
