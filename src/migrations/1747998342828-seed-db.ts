import type { EntityManager, MigrationInterface, QueryRunner, Repository } from 'typeorm';
...
import type { ExistingEntity } from '../common/types/existing-entity.type';
import * as process from 'node:process';
import type { SwapiResourceItem } from '../common/interfaces/swapi-resource-item.interface';
import type { SwapiResource } from '../common/interfaces/swapi-resource.interface';
...
export class SeedDb1747998342828 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const resourceName of Object.keys(allResources)) {
      ...
      const repository: Repository<ExistingEntity> = this.getRepository(
        resourceName as RESOURCES_NAMES,
      );
      await this.insertItemsWithoutRelations(swapiData[resourceName], repository);
    }

    console.log(
      `[INFO] -- ${new Date().toISOString()} -- Inserted all items to database. Proceeding to add relations.`,
    );
    ...
    console.log(`[INFO] -- ${new Date().toISOString()} -- Added relations to all items!`);
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    console.log(`[INFO] -- ${new Date().toISOString()} -- Seeded data has been cleared!`);
  }
  ...
  private filterPrimitivePropsForItem(
    ...
    for (const prop of Object.keys(item)) {
      if (this.isPropertyPrimitive(item, prop)) {
        newItem[prop] = item[prop] as ExistingEntity[keyof ExistingEntity];
      }
    }
  ...
  private isPropertyPrimitive(item: SwapiResourceItem, propertyName: string): boolean {
    return (
      !Array.isArray(item[propertyName]) && !Object.keys(STR_PROPS_TO_IGNORE).includes(propertyName)
    );
  }
  ...
  private async addRelations(
    ...
    for (const item of items) {
      const itemWithRelations: ExistingEntity = await this.assignRelationsToUpdate(
        item,
        repository,
      );
  ...
  private async assignRelationsToUpdate(
    ...
    const itemWithoutRelations: ExistingEntity = await repository.findOneByOrFail({ id });
  ...
  private async assignArrayishRelations(
    ...
    const relationRepository: Repository<ExistingEntity> = this.getRepository(
      relationProperty as RESOURCES_NAMES,
    );

    return Promise.all(relationIds.map((id) => relationRepository.findOneByOrFail({ id })));
  }
  ...
  private getRepository<T extends ExistingEntity>(resourceName: RESOURCES_NAMES): Repository<T> {
