...
export class JwtRefreshStrategy extends PassportStrategy(Strategy, 'jwt-refresh') {
    ...
    async validate(req: ReqWithUserStrRoles, payload: JwtPayload): Promise<UserOnReq> {
        const refreshTokenInput: string | null = ExtractJwt.fromAuthHeaderAsBearerToken()(req)
        ...
        const isMatch: boolean = await bcrypt.compare(refreshTokenInput, refreshToken)

        if (!isMatch) {
            throw new UnauthorizedException('', 'Refresh token does not match local one.')
        }
